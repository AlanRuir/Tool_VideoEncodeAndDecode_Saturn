...
while true; do
    ...
    ffmpeg -y -hide_banner -loglevel panic -hwaccel "$hwaccel" -i "$mp4_file" -vf "select='eq(n,$frame_index)',format=yuv420p" -vframes 1 -vsync vfr "$output_path/$frame_index.yuv"
